...
import { fetchProjetos, salvarCustoRealizado, fetchHistoricoPrecoItem } from '@/modules/operacional/services/apiProjetos';
...
interface CustoLancarModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}
  ...
  const handleReceiptProcess = async (images: string[], qrCodeUrl: string | null) => {
    ...
    try {
      ...
      if (data && data.notaFiscal) {
        let nf: NotaFiscal = data.notaFiscal;
        
        // Buscar histórico de preços para cada item
        if (nf.itens && nf.itens.length > 0) {
          const itensComHistorico = await Promise.all(
            nf.itens.map(async (item) => {
              const precoAnterior = await fetchHistoricoPrecoItem(item.nome_item);
              if (precoAnterior && precoAnterior > 0) {
                const variacao = ((item.valor_unitario - precoAnterior) / precoAnterior) * 100;
                return { ...item, variacao, preco_anterior: precoAnterior };
              }
              return item;
            })
          );
          nf.itens = itensComHistorico;
        }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-card w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-card-border shrink-0">
          <div>
            <h2 className="text-xl font-bold font-vomzom text-main flex items-center gap-2">
              <Coins className="text-brand-ocre" size={24} /> Registrar Custo Efetivo
            </h2>
            <p className="text-xs text-sub mt-1">Lançamento de nota fiscal ou despesa manual</p>
          </div>
          <button 
            onClick={onClose}
            className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-zinc-800 text-sub hover:text-main transition-colors"
          >
            <X size={20} />
          </button>
        </div>
                      ...
                      <div className="flex justify-between items-start sm:block mb-2 sm:mb-0">
                        <div className="font-medium text-sm sm:text-xs text-main flex-1 mr-2">
                          ...
                          {item.variacao !== undefined && (
                            <div className={`text-[10px] font-bold mt-1 ${item.variacao > 0 ? 'text-red-500' : item.variacao < 0 ? 'text-green-500' : 'text-slate-500'}`}>
                              {item.variacao > 0 ? `↑ ${item.variacao.toFixed(1)}% mais caro` : item.variacao < 0 ? `↓ ${Math.abs(item.variacao).toFixed(1)}% mais barato` : `= Mesmo preço`}
                              <span className="text-sub font-normal ml-1">que o último (R$ {item.preco_anterior?.toFixed(2)})</span>
                            </div>
                          )}
                        </div>
